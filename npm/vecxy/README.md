...
- `vecxy engine install [ref]` installs or updates an engine version (`develop` by default).
- `vecxy engine use <ref>` installs and selects the default engine without changing a project.
- `vecxy engine use <ref> -p <path>` explicitly selects a version for one project.
...
selection is only changed by `engine use -p` and is stored in `.vecxy/config.json`. Its generated MSBuild
