...
public static partial class AssetPipeline
{
    ...
    public static AssetManifest Scan(string projectDirectory)
    {
        ...
        ScanDirectory("Game", assetsDirectory, entries, oldByPath, oldByHash, claimed, packages);
        var engineAssetsDirectory = FindEngineAssetsDirectory(root);
        if (engineAssetsDirectory is not null)
            ScanDirectory("Engine", engineAssetsDirectory, entries, oldByPath, oldByHash, claimed, null);
    ...
    public static string GenerateSource(AssetManifest manifest)
    {
        ...
        foreach (var package in manifest.Packages.Where(x => x.Id != PackageId.Game).OrderBy(x => x.Name, StringComparer.Ordinal))
        {
