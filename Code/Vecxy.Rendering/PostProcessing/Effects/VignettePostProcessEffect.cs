...
public sealed class VignettePostProcessConfig : APostProcessConfig
{
    ...
    public Vector4 GetColor(string path)
    {
        if (Color is null)
        ...
        if (Color.Length != 4)
        {
            throw new InvalidDataException(
                $"Vignette post process config '{path}' color must contain four components.");
        }
    ...
    public override void Validate(string path)
    {
        ...
        {
            throw new InvalidDataException(
                $"Vignette post process config '{path}' intensity must be between 0 and 1.");
        }

        if (RoundRadius < 0.0f || RoundRadius > 2.0f)
        {
            throw new InvalidDataException(
                $"Vignette post process config '{path}' roundRadius must be between 0 and 2.");
        }

        if (RoundSoftness <= 0.0f || RoundSoftness > 1.0f)
        {
            throw new InvalidDataException(
                $"Vignette post process config '{path}' roundSoftness must be between 0 and 1.");
        }

        ValidateSide(Top, nameof(Top), path);
        ValidateSide(Right, nameof(Right), path);
        ValidateSide(Bottom, nameof(Bottom), path);
        ValidateSide(Left, nameof(Left), path);

        if (EdgeSoftness <= 0.0f || EdgeSoftness > 0.25f)
        {
            throw new InvalidDataException(
                $"Vignette post process config '{path}' edgeSoftness must be between 0 and 0.25.");
        }
    }
        ...
        string path)
    {
        ...
        {
            throw new InvalidDataException(
                $"Vignette post process config '{path}' {name} must be between 0 and 1.");
        }
    }
}
    ...
    : APostProcessEffect<VignettePostProcessConfig>
{
    ...
    public override void Apply(Shader shader, in PostProcessContext context)
    {
        ...
        shader.Set(
            "uVignetteColor",
            settings.GetColor(Config?.Path ?? "<defaults>"));
