...
internal sealed class AssetPackageManager : IAsyncDisposable
{
        ...
        try
        {
            if (state.References == 0)
            {
                ...
                try
                {
                    ...
                    var path = Path.Combine(_directory, state.File ?? state.Package.Name.ToLowerInvariant() + ".vpack");
    ...
    { if(manifest.FormatVersion!=VPackFormat.Version)throw new NotSupportedException($"Unsupported package manifest version {manifest.FormatVersion}."); foreach(var entry in manifest.Packages) GetState(entry.Id).File=entry.File; }
    ...
    public async ValueTask DisposeAsync() { foreach(var state in _states.Values){if(state.Reader is not null)await state.Reader.DisposeAsync();state.Gate.Dispose();} }
    internal sealed class State(AssetPackage package) { public AssetPackage Package { get; }=package; public SemaphoreSlim Gate { get; }=new(1,1); public int References; public string? File; public VPackReader? Reader; public List<AssetPackageLease> DependencyLeases=[]; }
}
